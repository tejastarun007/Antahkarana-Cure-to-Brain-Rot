...
/**
 * Returns today's Gita teaching based on the current date.
 * Cycles through all teachings so each day of the month gets a unique one.
 */
export function getTodaysTeaching(): GitaTeaching {
  const now = new Date();
  // Use day-of-year for maximum spread
  const start = new Date(now.getFullYear(), 0, 0);
  const diff = now.getTime() - start.getTime();
  const dayOfYear = Math.floor(diff / (1000 * 60 * 60 * 24));
  const idx = dayOfYear % GITA_TEACHINGS.length;
  return GITA_TEACHINGS[idx];
}
